...
def compute_separability_metrics(
	...
	silhouette = silhouette_score(X, y) if len(classes) >= 2 else np.nan
...
def process_tensor_file(
	...
	if k_dirs is not None:
		for k, knn_dir in sorted(k_dirs.items()):
			adj_file, search_path = find_adj_file(knn_dir, meta.model, meta.dataset_label, meta.split, meta.epoch, k, meta.layer)
