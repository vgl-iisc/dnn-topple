...
def render_experiment_selector(id: int, experiments: list):
    ...
    with st.container(horizontal=True, vertical_alignment="center", gap="medium") as c:
        ...
        possible_ks = sorted(list(set([exp.k for exp in possible_experiments if exp.dataset.name == selected_dataset and exp.split == selected_split and exp.model == selected_model])))
        if len(possible_ks) > 1:
            selected_k = st.selectbox("k", options=possible_ks, key=f"experiment_k_selector_{id}", on_change=invalidate_experiment_view)
        else:
            selected_k = possible_ks[0] if possible_ks else None
            st.session_state[f"experiment_k_selector_{id}"] = selected_k
