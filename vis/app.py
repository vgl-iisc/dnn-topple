...
from vis_utils import find_steady_simplification_states, compute_arc_features, RichFeature
...
def render_experiment(exp: LossLandscapeExperiment):
    ...
    with st.container(horizontal=True, horizontal_alignment="center") as c:
        ...
        if st.button("Compute Arcs and Coverage", key=f"compute_button_{repr(exp)}"):
            st.session_state[f"feats_{repr(exp)}"] = compute_arcs_and_coverage(exp, simpl)

    # arc_col, cov_col = st.columns([3, 2])
    # with arc_col:
    #     st.subheader("Arc Explorer")
        
    #     render_arc_explorer(exp, simpl)
        
    # with cov_col:
    #     st.subheader("Coverage Map")
        
    #     render_coverage_map(exp, simpl)
...
def main():
    ...
    if st.session_state.selected_experiments is None or len(st.session_state.selected_experiments) == 0:
        st.info("Select one or more experiments to explore.")
        return
